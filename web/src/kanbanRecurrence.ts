...
export const DEFAULT_RECURRING_TITLES: Record<string, string> = {
  recurring_open: "Recurring: Open",
  recurring_progress: "Recurring: In Progress",
  recurring_done: "Recurring: Done",
};
...
export function buildRecurrence(
  ...
  dueDate: string,
  until: string,
): KanbanRecurrence | null {
  if (!freq) return null;
  const anchor = isIsoDate(dueDate) ? dueDate : null;
  const now = new Date();
  const y = anchor ? Number(anchor.slice(0, 4)) : now.getFullYear();
  const m = anchor ? Number(anchor.slice(5, 7)) : now.getMonth() + 1;
  const d = anchor ? Number(anchor.slice(8, 10)) : now.getDate();
  const untilDate = isIsoDate(until) ? until : null;
  if (freq === "weekly") {
    const days =
      weekdays.length > 0
  ...
  if (freq === "monthly") {
    return { freq, weekdays: [], day: d, month: null, until: untilDate };
  }
  if (freq === "yearly") {
    return { freq, weekdays: [], day: d, month: m, until: untilDate };
  }
  return { freq, weekdays: [], day: null, month: null, until: untilDate };
}
