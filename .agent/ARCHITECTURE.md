...
- **Kanban is not tldraw.** Own tables (`kanban_boards/columns/cards/members/groups`, plus `kanban_labels`, `kanban_card_assignees`, `kanban_card_labels`), REST mutations, WebSocket `/api/kanban-sync/:id` broadcasts full state after each change. Access matches whiteboards (admins always; members via people + groups). Cards carry priority, optional `dueDate` (`YYYY-MM-DD` or null), assignees, and labels in that snapshot. Board page has Board / Calendar layouts (`?view=calendar`); calendar is month-grid of due dates plus an undated tray. Same filters, same live state. Gantt not built.
