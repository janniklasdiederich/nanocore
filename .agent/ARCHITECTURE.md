...
- **Kanban is not tldraw.** Own tables (`kanban_boards/columns/cards/members/groups`, plus `kanban_labels`, `kanban_card_assignees`, `kanban_card_labels`), REST mutations, WebSocket `/api/kanban-sync/:id` broadcasts full state after each change. Access matches whiteboards (admins always; members via people + groups). Cards carry priority, optional `dueDate` (`YYYY-MM-DD` or null), assignees, labels, and comments in that snapshot. Anyone with board access can comment; author or org admin can delete. Board page has Board / Calendar layouts (`?view=calendar`); calendar is Day / Week / Month (`cal=`) of due dates plus an undated tray. Same filters, same live state. Gantt not built.
